...
- **Presión mínima:** Hay dos, con distinto alcance:
  - **Componentes y revestimientos:** ±800 N/m² (Art. 5.2.2, `PRESION_MINIMA_COMPONENTES` en `presiones/edificio.py`), aplicado al módulo de cada signo del valor neto, incluyendo las paredes bajo la Figura 8 y el alero. Ojo al escribir tests: en edificios chicos el recorte tapa las diferencias entre zonas, así que un test que compare zonas necesita una velocidad alta.
...
- **Estado de la migración a 102-2025:** Migrados la presión dinámica (Art. 1.13), mapas (Fig. 1.5-1), $K_z$ (Tabla 1.13-1), factor topográfico (Art. 1.8), ráfaga (Art. 1.9), altitud (Art. 1.12), el SPRFV por método direccional (Fig. 2.4-1), las paredes de C&R de edificios bajos (Tabla C 5.3-1), la cubierta de C&R a dos aguas con θ ≤ 7° y h ≤ 20 m (Tabla C 5.3-2 / Fig. 5.3-2A completa, con las Zonas 1', 1, 2 y 3 medidas con h y las Notas 5, 6 y 7), las Tablas C 5.3-3, 5.3-4 y 5.3-5 (Figs. 5.3-2B, C y D) y la cubierta a un agua de C&R para 3° < θ ≤ 10° y h ≤ 20 m (Fig. 5.3-5A, con θ ≤ 3° a la 5.3-2 sobre el faldón único). **Pendiente** (marcado con `TODO` en el código): el resto de las figuras de componentes de cubierta (5B/7A para otros ángulos o alturas y 8) y la rama de paredes de gran altura de C&R (Figura 8). Además falta la zona 4⁺ de la superficie inferior de edificios separados del suelo (Nota 8 / Fig. 5.3-1A), que está en el issue #9.
