...
- **Vista 3D de componentes:** Los actores van por zona (`ZonaComponenteParedEdificio` / `ZonaComponenteCubiertaEdificio`) y están todos visibles; lo que cambia con la interfaz es sólo el valor de presión, según componente, zona y tipo de presión. La presión positiva se lee de la zona `TODAS`.
