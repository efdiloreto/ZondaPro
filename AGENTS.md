...
- **Estado de la migración a 102-2025:** Migrados la presión dinámica (Art. 1.13), mapas (Fig. 1.5-1), $K_z$ (Tabla 1.13-1), factor topográfico (Art. 1.8), ráfaga (Art. 1.9), altitud (Art. 1.12), el SPRFV por método direccional (Fig. 2.4-1) y las paredes de C&R de edificios bajos (Tabla C 5.3-1). **Pendiente** (marcado con `TODO` en el código): las figuras de componentes de cubierta (5B/7A/8), la rama de paredes de gran altura de C&R (Figura 8) y la presión mínima de componentes (Art. 1.4.2), que siguen CIRSOC 102-2005.
