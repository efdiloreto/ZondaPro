...
def _seleccionar_dato(combobox: QtWidgets.QComboBox, dato) -> None:
    """Selecciona en el combobox la opción cuyo ``userData`` es ``dato``.

    Args:
        combobox: El combobox donde buscar.
        dato: El dato de la opción a seleccionar.

    Raises:
        ErrorArchivo: Si el combobox no tiene ninguna opción con ese dato.
    """
    indice = combobox.findData(dato)
    if indice < 0:
        raise excepciones.ErrorArchivo(
            f'El archivo tiene un valor que esta pantalla no admite: "{dato}".'
        )
...
class WidgetComponentes(QtWidgets.QTableWidget):
    ...
    def __init__(self, componentes: dict[str, float] | None = None):
        super().__init__()
        self.setColumnCount(2)
        self.setRowCount(30)
        # Los headers de un QTableWidget existen siempre; los stubs de PyQt6 los
        # declaran opcionales igual.
        header_vertical = self.verticalHeader()
        assert header_vertical is not None
        header_vertical.setDefaultSectionSize(22)
        ...
        assert header_horizontal is not None
        header_horizontal.setStretchLastSection(True)
        header_horizontal.setSectionResizeMode(QtWidgets.QHeaderView.ResizeMode.Stretch)
        ...
        if componentes is not None:
            for i, (descripcion, area) in enumerate(componentes.items()):
                self._celda(i, 0).setText(descripcion)
                self._celda(i, 1).setText(str(area))

    def _celda(self, fila: int, columna: int) -> QtWidgets.QTableWidgetItem:
        """La celda de la tabla, que ``__init__`` llena de punta a punta."""
        item = self.item(fila, columna)
        assert item is not None
        return item

    def componentes(self) -> dict[str, float] | None:
        ...
        for fila in range(self.rowCount()):
            nombre = self._celda(fila, 0).text()
            area_str = self._celda(fila, 1).text()
            if nombre:
...
class WidgetEstructuraBase(QtWidgets.QWidget):
    """Base de las pantallas de entrada de cada tipo de estructura.

    Además de los datos que consume el motor de cálculo (``parametros()``), cada
    pantalla sabe leer y escribir su propio estado (``estado()`` y
    ``cargar_estado()``), que es lo que se guarda en el archivo de proyecto.

    Mientras se carga un estado, ``_cargando`` queda en ``True``: cambiarle el
    valor a cada widget dispara sus señales, y sin la bandera la escena 3D se
    rearmaría una vez por widget —y el aviso de parapeto se abriría solo—.
    """
    ...
    def _cargar_spinboxs(self, spinboxs: dict, valores: dict) -> None:
        """Le asigna a cada spinbox el valor guardado con su misma clave."""
        for nombre, spinbox in spinboxs.items():
            spinbox.setValue(float(valores[nombre]))
...
class WidgetEstructuraEdificio(WidgetEstructuraBase):
    def __init__(self):
        ...
        self._checkbox_alero.setLayoutDirection(QtCore.Qt.LayoutDirection.RightToLeft)
        ...
        self._checkbox_parapeto.setLayoutDirection(
        ...
        self._grid_layout_geometria.addWidget(
            QtWidgets.QLabel("Tipo de Cubierta"),
            0,
            0,
        ...
        self._grid_layout_geometria.addWidget(
            QtWidgets.QLabel("Personalizar Alturas"),
            8,
            0,
            QtCore.Qt.AlignmentFlag.AlignRight,
        )
        ...
        for (f, c), (key, spinbox) in zip(
            ...
            grid_layout_aberturas.addWidget(
                QtWidgets.QLabel(key), f, c, QtCore.Qt.AlignmentFlag.AlignRight
            )
        ...
        self._grid_layout_reduccion_gcpi.addWidget(
            self._spinbox_volumen, 1, 1, QtCore.Qt.AlignmentFlag.AlignLeft
        )
    ...
    def _habilitar_deshabilitar_volumen(self, estado: bool) -> None:
        ...
        elif self._volumen_usuario is not None:
    ...
    def _validar(self) -> bool:
        ...
        # Anidado a proposito: separa la validacion del tipo de cubierta.
        if tipo_cubierta != TipoCubierta.PLANA:  # noqa: SIM102
    ...
    def estado(self):
        """El estado de la pantalla, tal como se guarda en el archivo."""
        return {
            "tipo_cubierta": self._combobox_tipo_cubierta.currentData(),
            "geometria": {
            ...
            "aberturas": {
                ...
                for nombre, spinbox in self._spinboxs_aberturas.items()
            },
            "reducir_gcpi": self._box_reduccion_gcpi.isChecked(),
            "unico_volumen": self._checkbox_unico_volumen.isChecked(),
            "volumen": self._spinbox_volumen.value(),
        }

    def cargar_estado(self, estado) -> None:
        """Deja la pantalla como la dejó ``estado``.

        El volumen se asigna al final y a propósito: ``_generar_escena()`` lo
        pisa con el volumen geométrico del edificio, y el que manda es el que
        estaba guardado.
        """
        self._cargando = True
        try:
            _seleccionar_dato(self._combobox_tipo_cubierta, estado["tipo_cubierta"])
            geometria_guardada = estado["geometria"]
            self._spinboxs["alero"].setMaximum(
                float(geometria_guardada["altura_alero"])
            )
            self._cargar_spinboxs(self._spinboxs, geometria_guardada)
            ...
            self._cargando = False
        self._generar_escena()
        self._spinbox_volumen.setValue(float(estado["volumen"]))
        self._checkbox_unico_volumen.setChecked(estado["unico_volumen"])
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i in range(4):
            ...
            for j in range(1, 5):
                grid_layout.addWidget(
                    QtWidgets.QLabel("="), j, 1, QtCore.Qt.AlignmentFlag.AlignCenter
                )
            ...
            grid_layout.addWidget(
                label, 1, 4, 4, 1, QtCore.Qt.AlignmentFlag.AlignCenter
            )
