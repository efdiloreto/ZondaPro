...
if TYPE_CHECKING:
    from zonda.tipos import ParNumerico, Punto, Punto2D
...
def punto_sobre_vector(
    ...
    _fin = np.array(fin)
    vector = _fin - _origen
    norm_esc = vector / np.linalg.norm(vector) * distancia
...
def coords_pared_rectangular(
    ...
    Args:
        ...
        invertir_sentido: Indica si los puntos se tiene que retornar en el sentido inverso. Es util para cuando se quiere
            que la normal al poligono apareza de un lado o del otro.
...
def coords_zona_cubierta(
    ...
    """Crea las coordenadas de un sector de cubierta.
...
def coords_zona_cubierta_desde_proyeccion(
    ...
    """Crea las coordenadas de un sector de cubierta, desde la proyeccion
    ...
    dist_inicio_proyectada = float(
        np.linalg.norm(array_punto_inicio_proyectado - array_origen)
    )
    dist_fin_proyectada = float(
        np.linalg.norm(array_punto_fin_proyectado - array_origen)
    )
