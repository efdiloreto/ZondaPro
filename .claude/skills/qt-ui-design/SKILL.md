...
## 7. Adaptación a Zonda (agregado local, no viene del upstream)

Todo lo anterior es contenido original de The Qt Company. Esta sección la agregó
el proyecto para acotar el alcance; si se actualiza la skill desde el upstream,
hay que volver a agregarla.

**Contexto ya resuelto — no preguntarlo (secciones 0.1 a 0.7):**

| Ítem | Valor en Zonda |
|---|---|
| Plataforma | Escritorio (Windows/Linux/macOS), **PyQt6 Widgets** para la interfaz y **Qt Quick 3D** (QML) para la vista 3D |
| Forma de pantalla | Rectangular, ventana redimensionable |
| Distancia de visión | ~60 cm (escritorio) |
| Sistema de diseño | Estilo nativo de Qt Widgets; sin design system propio ni tokens de Figma |
| Locale | Español (`es-AR`), LTR. **No hace falta soporte RTL** |
| Entrada | Mouse + teclado. No hay táctil ni botones de hardware |
| Prioridad de contenido | Entrada de datos de la estructura → resultados numéricos → gráfico 3D → reporte |

**Qué aplica y qué no:**

- **Aplican:** sección 1 (principios de diseño y layout), 1.3 (navegación por
  teclado y foco), 1.4 (color semántico), 2 (accesibilidad WCAG), 5 (auditoría,
  checklist de escritorio).
- **Ignorar por completo:** sección 4 (embedded/MCU) y sección 3 (UX de IA) —
  el programa no tiene features de IA ni corre en hardware embebido.
- **Traducir, no copiar:** todos los ejemplos de implementación están en QML.
  El único QML del proyecto es `zonda/graficos/Visor.qml`, la vista 3D; la
  interfaz es Qt Widgets. No existe `TypeScale.qml`, ni `qmldir`, ni
  `qt_add_qml_module`, ni `LayoutMirroring`. Los equivalentes son:

| Concepto de la skill (QML) | Equivalente en Zonda (PyQt6 Widgets) |
|---|---|
| Singleton `TypeScale.qml` | Constantes en un módulo Python de `zonda/widgets/` |
| `font.pointSize` | `QFont.setPointSize()` / hoja de estilo con `pt`, nunca `px` |
| Tokens de color en singleton | Constantes + `QPalette` / hoja de estilo Qt |
| `activeFocus` para el indicador de foco | Foco nativo de Qt Widgets; no lo deshabilites |
| Animaciones `transform`/`opacity` | `QPropertyAnimation`; en general **no** hacen falta |
| `QLocale` desde QML | `QLocale` desde Python, para números y fechas |

- **Idioma:** los textos de interfaz, comentarios y nombres nuevos van en
  español, igual que el resto del código (ver `AGENTS.md`).
- **Unidades y notación:** los valores de ingeniería (presiones, alturas,
  ángulos) se muestran con su unidad y con la cantidad de decimales que ya usa
  `zonda/widgets/resultados.py`. No cambies el formato numérico sin pedido
  explícito.
